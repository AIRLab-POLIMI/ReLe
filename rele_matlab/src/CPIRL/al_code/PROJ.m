...
function [PP, MM, ITER, TT] = PROJ(THETA, F, GAMMA, T, E, INIT_FLAG, tol)
if nargin < 7
    tol = 0.0001;
end
...
for i=2:(T+1)
    ...
    [PP(i-1, :), MM(i-1, :), VV, ITER(i-1)] = opt_policy_and_feat_exp(THETA, F, GAMMA, w(i-1, :)', INIT_FLAG, VV, tol);
    if (i == 2)
        BM(1, :) = MM(1, :);
        w(2, :) = E - MM(1, :);
    else
