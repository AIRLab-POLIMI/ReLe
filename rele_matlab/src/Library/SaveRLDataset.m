function SaveRLDataset( filename, data, type, reward_idx, continuous )

% function SaveRLDataset
%
% Inputs:
%
...
%  - rewardidx  : indices of the rewards to be saved (DEFAULT all)
...
if type == 2
	dlmwrite(filename, [length(data(1).s(:,1)) + length(data(1).a(:,1)) length(data(1).s(:,1))+length(reward_idx)], '-append', 'delimiter','\t','precision', 8);
end
...
for ep = 1:nepisodes
    ...
    for sample = 1:nsamples
        ...
        if type == 1
            dlmwrite(filename, [state', action', reward(reward_idx), endep], '-append', 'delimiter','\t','precision', 8);
        else
            ...
            dlmwrite(filename, [state', action', nextstate', reward(reward_idx)'], '-append', 'delimiter','\t','precision', 8);
        end
    end
end
