...
for dim = 2%[2,5,10,20]
    ...
    mkdir(['~/Dropbox/EWRL2015_irl/tests/LQRMLEPol3deg/',num2str(dim),'obj']);
...
%                 results(i).gnorm_eval{kk} = [];
            ...
            for run = 1:3
                ...
                delete('/tmp/ReLe/lqr/MLEALL1D/*');
                
                %             algorithm = 'rb';
                cmd = '/home/matteo/Projects/github/ReLe/rele-build/lqr_MLEGIRLALL1D';
                ...
                for kk = 1:length(gtypes)
                    algorithm = gtypes{kk};
                    plane_R = dlmread(['/tmp/ReLe/lqr/MLEALL1D/girl_plane_',algorithm,'.log']);
%                     gnorm_R = dlmread(['/tmp/ReLe/lqr/MLEALL1D/girl_gnorm_',algorithm,'.log']);
%                     gnorm_eval = dlmread(['/tmp/ReLe/lqr/MLEALL1D/girl_gnorm_',algorithm,'_neval.log']);
                    
                    A = dlmread(['/tmp/ReLe/lqr/MLEALL1D/girl_time_',algorithm,'.log']);
%                     gnorm_T = A(1,:);
                    plane_T = A(1,:);
                    clear A;
...
%                 cmdtar = ['tar cvzf ~/Dropbox/EWRL2015_irl/tests/LQRMLEPol3deg/',num2str(dim),'obj/lqr_',num2str(nbEpisodes),'_',...
%                     num2str(dim),'_',num2str(run),'.tar.gz -C  /tmp/ReLe/lqr/ GIRL'];
%                 status = system(cmdtar);
                
            end
            
            sss = ['/home/matteo/Dropbox/EWRL2015_irl/tests/LQRMLEPol3deg/',num2str(dim),'obj/tmpGIRL',num2str(dim),'D_',num2str(dim),'dim.mat'];
            save(sss)
    ...
    sss = ['/home/matteo/Dropbox/EWRL2015_irl/tests/LQRMLEPol3deg/',num2str(dim),'obj/GIRL',num2str(dim),'D_',num2str(dim),'dim_fixS_sqrt2.mat'];
    save(sss);
    % save /home/matteo/Dropbox/EWRL2015_irl/tests/LQRMLEPol3deg/2obj/GIRL2D_2dim.mat
