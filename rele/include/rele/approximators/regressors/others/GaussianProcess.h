...
public:
	...
	virtual void trainFeatures(const BatchData_<arma::vec, denseOutput>& featureDataset) override
	{
		...
		arma::mat eye = arma::eye<arma::mat>(K.n_rows, K.n_cols);

		arma::mat chol = K + hParams.noiseVariance * hParams.noiseVariance * eye;
		L = arma::chol(chol, "lower");
...
protected:
	arma::mat generateCovMatrix(typename input_traits<denseOutput>::type testFeatures)
	{
		...
		for(unsigned int i = 0; i < K.n_rows; i++)
			for(unsigned int j = i; j < K.n_cols; j++)
				K(i, j) = K(j, i) = computeKernel(testFeatures.col(i), testFeatures.col(j));
	...
	double computeKernel(arma::vec x_p, arma::vec x_q)
	{
		double k = 0;

		if(covFunction == rbf)
			k = hParams.signalVariance * hParams.signalVariance *
					exp(- 0.5 * arma::norm((x_p - x_q)) * arma::norm((x_p - x_q)) /
					(hParams.lengthScale * hParams.lengthScale));
		else
		{
