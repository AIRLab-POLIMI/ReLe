...
public:
    ...
    virtual void run()
    {
        ...
        std::vector<double> parameters(dpr, 0.0/dpr);
        double minf;
    ...
    arma::vec ReinforceBaseGradient(arma::mat& gGradient)
    {
        ...
        for (int p = 0; p < dp; ++p)
        {
            ...
            for (int ep = 0; ep < nbEpisodes; ++ep)
            {
                ...
                for (int rp = 0; rp < dpr; ++rp)
                {
                    gGradient(p,rp) += (return_R_ObjEp[ep](0,rp) - baseline_R_num(p,rp)) * sumGradLog_CompEp(p,ep);
                }
            }
        }
    ...
    double objFunction(unsigned int n, const double* x, double* grad)
    {
        ...
        //        std::cerr << rewardf.getParameters().t();
        if (atype == AlgType::R)
        {
