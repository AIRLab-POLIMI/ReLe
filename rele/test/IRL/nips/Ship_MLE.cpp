...
int main(int argc, char *argv[])
{
    ...
    FileManager fm("nips/ship_mle/"+n_episodes+"/"+n_experiment);
    fm.cleanDir();
    fm.createDir();
    ...
    p.save("/tmp/ReLe/nips/ship_mle/ExpertParams.txt",  arma::raw_ascii);
#else
    arma::vec p;
    p.load("/tmp/ReLe/nips/ship_mle/ExpertParams.txt", arma::raw_ascii);
#endif
    ...
    //Recover expert distribution
    std::cout << "Recovering Distribution" << std::endl;

    auto basisImitator = PolynomialFunction::generate(1, dim);
    DenseFeatures phiImitator(basisImitator);

    arma::mat SigmaPolicy = arma::eye(1, 1)*1e-3;
    ...
    irlAlg[0] = new EGIRL<DenseAction,DenseState>(data, theta, imitatorDist, rewardRegressor, mdp.getSettings().gamma, IrlEpGrad::PGPE_BASELINE);
    irlAlg[1] = new SDPEGIRL<DenseAction,DenseState>(data, theta, imitatorDist, rewardRegressor, mdp.getSettings().gamma, IrlEpGrad::PGPE_BASELINE, IrlEpHess::PGPE_BASELINE);
    irlAlg[2] = new CurvatureEGIRL<DenseAction,DenseState>(data, theta, imitatorDist, rewardRegressor, mdp.getSettings().gamma, IrlEpGrad::PGPE_BASELINE, IrlEpHess::PGPE_BASELINE, 1e-4);
